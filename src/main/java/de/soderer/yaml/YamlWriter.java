...
public class YamlWriter implements Closeable {
	...
	public void writeDocument(final YamlDocument doc) throws IOException {
		for (final String c : doc.getLeadingComments()) {
			write("# ");
			write(c);
			write("\n");
		}
		...
		if (doc.getRoot() != null) {
			writeNode(doc.getRoot(), 0, false, false);
		}
